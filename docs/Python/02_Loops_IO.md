...
tabs in one part and spaces in another.
...
`!=` for not equal and `is not` for corollary test with `is`.
...
**Lists** are sets of values, strings, numbers. Initialize with a `[ ]` around the values. **Tuples** are sets of values but they are
