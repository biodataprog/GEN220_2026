...
Sometimes we want to match not a single equality but a pattern. Mostly
...
The `r` in front of a pattern string (e.g. `r'\d bird'`) makes it a *raw string*,
so the backslash is passed to the regular expression unchanged.

```python
re.search(r'\d bird', '8 birds') # true
re.search(r'\d bird', '1 bird') # true
re.search(r'\d bird', 'A bird') # false

re.search(r'[123] bird', '1 bird')  # true
re.search(r'[0-3] bird', '4 birds') # false

re.search(r'\d bird', '4 Birds') # false
re.search(r'\d [Bb]ird', '4 Birds') # true
```
...
```python
re.search(r'\d birds?','8 birds') # true
re.search(r'\d birds?','1 bird') # true
...
```python
re.search(r'\d bird', '8 birds') # true
re.search(r'\d bird$', '8 birds') # false
re.search(r'^\d bird', '8 birds') # true
re.search(r'^\d bird', '10 birds') # false
```

### Pattern searching

If you want to find more than one occurrence, or
count the number of occurrences you can use `search` or `findall`
options

```python
start = 0
pat = re.compile(pattern)
m = pat.search(string, start)
while( m ):
    #  process this match
    start = m.end()+1
    m = pat.search(string, start)
...
The `re.sub()` function allows replacement
