...
The structure requires the `[ ]` and there is expected to be a space between the `[` or `]` and the options; The `then` is also required but you can compact this slightly differently.
...
More logical operators such as testing if a number is smaller or greater with `-gt` and `-lt`.
...
Can use the `seq` function to make a list of numbers. Arguments are either the ending number, or start and end, or start, increment (offset), and end.
...
Capturing output from a program is also useful. For example if you want to do simple mathematical  arithmetic with the UNIX tool `expr` (or "evaluate expression"). It takes arguments for simple math.
...
[https://github.com/biodataprog/GEN220_data/tree/main/data](https://github.com/biodataprog/GEN220_data/tree/main/data)
...
* `-d`/`--dictionary-order` : consider only blanks & alphanumeric characters
...
# 9th column is the start position of the match in the subject (chromosome); Bitscore is column 12
...
```bash
$ awk '{print $1}' data/yeast_orfs-to-chr1.FASTA.tab   # print out the first column of a file
...
Here get the length of an alignment (column 6 is the START and column 7 is the end) using awk.
