...
# count all lines in multiple compressed files
$ zcat *.gz | wc -l
348854
```
...
```bash
$ gunzip file.fa.gz
# will produce file.fa
```

# Searching for text with grep

Powerful pattern searching with `grep`
...
Note that creating these same pairs on your local laptop and copying the public key to your HPCC account. Some basic info is here as well. [SSH keys](../Resources/SSH_keys)
...
Click on the GitHub Classroom links for homework submission posted in [Canvas](https://elearn.ucr.edu/):
...
![githubrepo](img/hw_github_repo2.png)
...
```bash
git clone git@github.com:biodataprog/2026-hw1-YOURUSERNAME.git
```
...
```bash
git clone https://github.com/biodataprog/2026-hw1-YOURNAME.git
```
